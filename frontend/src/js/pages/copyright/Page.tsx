...
export default () => {
    ...
    const [copyright_columns] = useState<GridColDef[]>([
        { field: 'actions', headerName: "Actions", renderCell: ({ row }) => <>
            ...
            <ActionPanelItem
                ...
                icon={EditIcon}
                onAction={() => {
                    console.log('EditCopyright '+row.organization)
                    console.log('EditCopyright ORG   '+row.organization.id )
                    update_add_copy({   
                        id: row.id,
    ...
    const [selectedResponseDate, handleResponseDateChange] = useState(null); 
                ...
                <Box display="flex">
                    ...
                    <Button
                        ...
                        onClick={_ => update_is_open(s => {
                            ...
                            update_add_copy(s => {s.id=0,s.description='',s.date_contacted='',s.date_of_response='',
                            s.granted=false,s.organization=0, s.user=user_id})
            ...
            <Grid item xs={12}>
                ...
                <GenericDialog
                    ...
                    actions={<>
                        ...
                        <Button
                            onClick={_ => {
                                console.log('SClick' + add_copy.id)
                                add_copy.id == 0 ?
              ...
              <MuiPickersUtilsProvider utils={DateFnsUtils}>
                ...
                <KeyboardDatePicker InputLabelProps={{ shrink: true }}
                  ...
                  //  format={"yyyy-MM-dd"}
                    ...
                    error={false}
                    helperText={null}
                    value={ add_copy.date_of_response ? moment( add_copy.date_of_response) : null}
                    //    const [selectedResponseDate, handleResponseDateChange] = useState(null); 
                   //value={add_copy.date_of_response || ""} 
                   onChange={e => update_add_copy(add_copy => {
                        console.log('SomethingRes')
                        console.log('-->'+ add_copy.date_of_response +" e " + e )
                        if(e !== null){
                            add_copy.date_of_response = e?.getFullYear()+'-'+ e?.getMonth()+'-'+e?.getDate() ?? ""
                           
                        }
                        else{
                            console.log( " ALTER " + add_copy.date_of_response)
                            add_copy.date_of_response = null;
                            console.log( " BELATER " + add_copy.date_of_response)
                            }
